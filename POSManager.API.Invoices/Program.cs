...
using POSManager.API.Invoices.Scope;
using Serilog.Core;
using Serilog;
...
namespace POSManager.API.Invoices;
...
public class Program
{
    private static async Task Main(string[] args)
    {
        ...
        try
        {
            ...
            {
                opts.UseSqlServer(builder.Configuration.GetConnectionString("Tenants"));
            });
            ...
            if (domain is not null)
            {
                ...
                {
                    options.AddPolicy("invoices.read", policy => policy.Requirements.Add(new HasScopeRequirement("invoices.read", domain)));

                    options.AddPolicy("invoices.write", policy => policy.Requirements.Add(new HasScopeRequirement("invoices.write", domain)));
                });
            }

            builder.Services.AddTransient<IOperationService>(s => new OperationService(typeof(Invoice)));
        ...
        catch (Exception error)
        {
            ...
            Log.Fatal(error, $"Invoices API crashed on start.");
        }
        finally
        {
            Log.Information("Invoices API closing...");
            Log.Information("====================================================================\r\n");
            Log.CloseAndFlush();
        }
